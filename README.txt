...
## Usage
`make` - for compiling and running
`make debug` - for compiling with debug symbols, then running gdb 
`make test` - for compiling with 'TEST' macro definition, utilizing Unity framework for tests, then runs the program with tests on top of valgrind.
